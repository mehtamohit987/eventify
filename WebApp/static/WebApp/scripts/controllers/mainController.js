    ...
    eventify.controller('mainController', ['$scope', '$http', '$location', 'AuthToken', function ($scope, $http, $location, AuthToken) {
        ...
        $scope.location_button = function() {
            ...
            $scope.$watch('coordinates', function() {
              AuthToken.updateSelectedCountry($scope.coordinates);
        ...
        $scope.search_button = function(){
            ...
            $scope.$watch('titleQuery', function() {
              AuthToken.updateTitleQuery($scope.titleQuery);
            });
            ///
            $scope.$watch('page', function() {
              AuthToken.updatePage($scope.page);
            });
            $scope.$watch('selected_time_range', function() {
              AuthToken.updateSelectedTimeRange($scope.selected_time_range);
            });
            $scope.$watch('date', function() {
              AuthToken.updateDate($scope.date);
            });
            $scope.$watch('selected_country', function() {
              AuthToken.updateSelectedCountry($scope.selected_country);
            });
            

            // AuthToken.page =  0;
            // //$main.titleQuery = AuthToken.titleQuery ;//     titleQuery: $scope.titleQuery,
            // AuthToken.selected_time_range = $scope.selected_time_range; //     selected_time_range: $scope.selected_time_range,
            // AuthToken.date = date; //     date: date,
            // AuthToken.coordinates = $scope.coordinates; //     coordinates: $scope.coordinates,
            // AuthToken.selected_country = $scope.selected_country;            
